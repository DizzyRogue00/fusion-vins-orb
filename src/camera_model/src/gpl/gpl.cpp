                ...
                double& centerX,double& centerY,double& radius)
{
	...
	double sum_xxx=0.0,sum_yyy=0.0,sum_xyy=0.0,sum_xxy=0.0;
	...
	for (int i = 0; i < n; ++i) {
    	double x = points.at(i).x;
    	double y = points.at(i).y;
    	sum_r += hypot(x - centerX, y - centerY);  // 计算欧氏距离
	}
	radius = sum_r / n;  // 半径的平均值
}
...
)
{
	...
	double a = (square(r1) - square(r2) + square(d)) / (2 * d);
