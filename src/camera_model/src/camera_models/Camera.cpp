...
#include "cameraModelCal/camera_models/ScaramuzzaCamera.h"
...
void 
Camera::estimateExtrinsic(const std::vector<cv::Point3f>& objectPoints,
...
{
    int imageCount=objectPoints.size();//图像数量
    size_t pointSoFar=0;//总点数计数器
    double totalErr=0.0;

    bool computePerViewErrors=_perViewErrors.needed();//是否需要计算是视角误差
...
{
    imagePoints.reserve(pbjectivePoints.size());
    ...
    for(size_t i=0;i<objectPoints.size();++i)
    {
        ...
        Eigen::Vector3d P;
        P << objectPoints.at(i).x, objectPoints.at(i).y, objectPoints.at(i).z;
